...
app.get(['/api/auth/google/login', '/auth/google/login'], (req, res) => {
  ...
  // Construct Redirect URI (guarantee https in production)
  const proto = (req.headers['x-forwarded-proto'] as string) || (req.get('host')?.includes('localhost') ? 'http' : 'https');
...
export async function assignLeadRoundRobinTransaction(
  ...
  // Normalized phone and email keys for deterministic deduplication
  ...
  return await targetDb.runTransaction(async (transaction: any) => {
    // 1. Check Durable Idempotency Key in Firestore
    if (idempotencyKey) {
    ...
    // 2. If candidate already exists and has an assigned_sales, do NOT overwrite unless management specifically provided an override
    if (candDoc.exists) {
    ...
    // 3. Deterministic Deduplication Lock Check in Firestore (Phone and Email locks)
    const phoneLockRef = cleanPhone.length >= 7 
      ? targetDb.collection('jpc_lead_locks').doc(`phone_${cleanPhone}`) 
      : null;
    const emailLockRef = cleanEmail.length >= 5 
      ? targetDb.collection('jpc_lead_locks').doc(`email_${cleanEmail.replace(/[^a-z0-9@._-]/g, '_')}`) 
      : null;

    const lockRefs: any[] = [phoneLockRef, emailLockRef].filter(Boolean);
      ...
      if (lockDoc.exists) {
        ...
        if (lockedCandId && lockedCandId !== candidateId) {
          ...
          if (lockedCandDoc.exists) {
            ...
            // Only consider duplicate if existing candidate is active (not soft-deleted)
    ...
    // Helper to persist deterministic locks & idempotency record within this atomic transaction
    const commitLocksAndIdempotency = (assignedSalesId: string | null, isUnassignedState: boolean, reasonStr?: string) => {
      const lockPayload = {
    ...
    // 4. Management explicit assignment override
    if (activeOverrideId) {
    ...
    // 5. Working hours validation (independent of scheduled deactivation)
    ...
    // 6. Read Round-Robin Configuration
    ...
    // 7. Query and filter eligible Sales Persons
    ...
    // 8. If no active sales reps, lead remains safely UNASSIGNED
    ...
    // 10. Calculate continuous round-robin pointer
    let nextIndex = 0;
...
/**
 * Submits an access request from outside the office.
 * Accessible even when the user is blocked from internal CRM routes.
 */
app.post(['/api/auth/request-external-access', '/auth/request-external-access'], async (req, res) => {
  try {
    const clientIp = extractClientIp(req);
    const {
      user_id,
    ...
    if (!user_id && !user_email && !username) {
      return res.status(400).json({ error: 'User identity is required to submit an access request' });
    }

    if (!reason || typeof reason !== 'string' || reason.trim().length === 0) {
      return res.status(400).json({ error: 'Please provide a reason for requesting outside-office access' });
    }

    // Try to resolve user document in jpc_users
    ...
    // Check if an existing pending request exists for this user
    const existingSnap = await db.collection('jpc_ip_access_requests')
...
/**
 * Checks the status of the current user's latest access request.
 */
...
/**
 * Lists all external access requests for Admin review.
 */
app.get(['/api/admin/ip-access/requests', '/admin/ip-access/requests'], verifyAdmin, async (req, res) => {
  try {
    const statusFilter = req.query.status as string; // 'pending', 'approved', 'rejected', or undefined
...
/**
 * Admin reviews (approves or rejects) an access request.
 * Approving can grant Global IP access OR restrict to request IP.
 */
app.post(['/api/admin/ip-access/requests/:id/review', '/admin/ip-access/requests/:id/review'], verifyAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      action,
      approved_scope = 'global',
      admin_notes = ''
    } = req.body;
    ...
    const reviewerName = (req as any).user?.display_name || (req as any).user?.username || 'Admin';
